...
-(void)setPhone:(NSString*)phone AndAreaCode:(NSString*)areaCode
{
    ...
    _showPhone = [NSString stringWithFormat:@"%@****%@",[phone substringWithRange:NSMakeRange(0, 3)],[phone substringWithRange:NSMakeRange(7, 4)]];
        ...
        [SMS_SDK commitVerifyCode:self.verifyCodeField.text result:^(enum SMS_ResponseState state) {
            if (1==state)
            {
                ...
                UIAlertView* alert=[[UIAlertView alloc] initWithTitle:NSLocalizedString(@"verifycoderighttitle", nil)
                                                    ...
                                                    otherButtonTitles:nil, nil];
                [alert show];
                _alert3 = [[UIAlertView alloc] init];
                _alert3=alert;
            }
            else if(0==state)
            {
...
-(void)CannotGetSMS
{
    NSString* str=[NSString stringWithFormat:@"%@:%@",NSLocalizedString(@"cannotgetsmsmsg", nil) ,_showPhone];
...
//        [self presentViewController:view2Controller animated:YES completion:^{
//            //解决等待时间乱跳的问题
//            [_timer2 invalidate];
//            [_timer1 invalidate];
//        }];
...
- (void)viewDidLoad
{
    ...
    //_phone = [[NSString alloc] initWithString:@""];
    NSLog(@"_phone %@",_phone);
    ...
    self.telLabel.text= [NSString stringWithFormat:@"+%@ %@",_areaCode,_showPhone];
...
-(void)tryVoiceCall
{
    UIAlertView* alert=[[UIAlertView alloc] initWithTitle:NSLocalizedString(@"verificationByVoiceCallConfirm", nil)
                                                  message:[NSString stringWithFormat:@"%@: +%@ %@",NSLocalizedString(@"willsendthecodeto", nil),_areaCode, _showPhone]
                                                 delegate:self
                                        cancelButtonTitle:NSLocalizedString(@"sure", nil)
