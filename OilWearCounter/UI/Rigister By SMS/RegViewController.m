...
-(void)nextStep
{
    NSLog(@"{nextStep} self.areaArray: %@",self.areaArray);
    int compareResult = 0;
    for (int i=0; i<self.areaArray.count; i++)
    {
        ...
        if ([code1 isEqualToString:[_areaCodeField.text stringByReplacingOccurrencesOfString:@"+" withString:@""]])
        {
            ...
            BOOL isMatch=[pred evaluateWithObject:self.savePhoneNumber];
            if (!isMatch)
            {
                //手机号码不正确
    ...
    if (!compareResult)
    {
        if (self.savePhoneNumber.length!=11)
        {
            //手机号码不正确
           UIAlertView* alert=[[UIAlertView alloc] initWithTitle:NSLocalizedString(@"notice", nil)
                                                         message:NSLocalizedString(@"errorphonenumber", nil)
    ...
    NSString* str=[NSString stringWithFormat:@"%@:%@ %@",NSLocalizedString(@"willsendthecodeto", nil),self.areaCodeField.text,self.showedPhoneNumber];
    _str=[NSString stringWithFormat:@"%@",self.savePhoneNumber];
...
-(void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
    if ( buttonIndex == 1 )
    {
        ...
        [verify setPhone:self.savePhoneNumber AndAreaCode:str2];
        NSLog(@"phone: %@",self.savePhoneNumber);
        
        [SMS_SDK getVerificationCodeBySMSWithPhone:self.savePhoneNumber
                                              zone:str2
...
//    [self.view addSubview:telField];
    ...
    telLabel.frame = CGRectMake(20+(self.view.frame.size.width - 30)/4, 155+statusBarHeight,(self.view.frame.size.width - 30)*3/4 , 40+statusBarHeight/4);
    ...
    self.savePhoneNumber = savePhoneNumber;
    NSLog(@"savePhoneNumber: %@",savePhoneNumber);
    self.showedPhoneNumber = [NSString stringWithFormat:@"%@****%@",[savePhoneNumber substringWithRange:NSMakeRange(0, 3)],[savePhoneNumber substringWithRange:NSMakeRange(7, 4)]];
    
    telLabel.text = self.showedPhoneNumber;
    telLabel.textAlignment = NSTextAlignmentCenter;
    telLabel.font = [UIFont fontWithName:@"Helvetica" size:18];
    telLabel.layer.cornerRadius = 20;
    telLabel.layer.borderColor = [UIColor grayColor].CGColor;
    telLabel.layer.borderWidth = 3;
    ...
    //NSLog(@"self.areaArray: %@",self.areaArray);
